...
// training state
const state={power:false,lineIso:true,cb132:true,busIso:true,cb33:true,feeders:[true,true,true],feederIso:[true,true,true],feederFault:[false,false,false],fault:false,cut:false,earth:false,training:false,protPulse:0};
...
document.getElementById('training').onclick=()=>{state.training=!state.training;document.getElementById('training').classList.toggle('trainingOn',state.training);document.getElementById('eqName').textContent=state.training?'TRAINING TASK':'TRAINING MODE';document.getElementById('eqInfo').textContent=state.training?'Task: safely de-energize the transformer. Start by opening the 33 kV incomer CB, then the 132 kV circuit breaker. Disconnectors must not interrupt load current.':'Training exercise stopped.';ui()};
// picking
